...
void loop() 
{
  ...
  if( (front_sonar > 0) && (front_sonar < 250.0))
  {
    ...
    motor_A_control(HIGH,200); //오른쪽으로 전진
    motor_B_control(LOW,200);  //왼쪽으로 후진
    delay (730);   
    //일정한 시간 동안 회전
 ...
 /*
    // 180도 반회전 시계 방향으러 회전
    Serial.println("Rotate CCW");
    motor_A_control(LOW, 100); //오른쪽으로 전진
    motor_B_control(HIGH,100);  //왼쪽으로 후진
    delay (530);
  ...
  */  
  ...
  //직진
  Serial.println("Go Straight");
  motor_A_control(HIGH,62); //왼쪽 바퀴
  motor_B_control(HIGH,80); //오른쪽 바퀴
  }
  delay(500);
}
